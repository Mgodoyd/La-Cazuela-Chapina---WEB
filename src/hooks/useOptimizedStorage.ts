...
export function useOptimizedStorage<T>(
  ...
  const { debounceMs = 300, validate = () => true } = options;
  ...
  useEffect(() => {
    const loadValue = async () => {
      try {
        ...
        console.error(
          `Error cargando valor de storage para ${storageKey}:`,
          error
        );
      } finally {
        setIsLoading(false);
    ...
    async (newValue: T) => {
      ...
      debounceRef.current = setTimeout(async () => {
        try {
          ...
          console.error(
            `Error guardando valor en storage para ${storageKey}:`,
            error
          );
        }
      }, debounceMs);
...
export function useCartStorage(userId: string | null) {
  const cartKey = useMemo(() => (userId ? `cart_${userId}` : null), [userId]);
  ...
  } = useOptimizedStorage<any[]>(cartKey || 'temp_cart', [], {
