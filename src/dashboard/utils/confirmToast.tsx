...
export function confirmToast(message: string): Promise<boolean> {
  return new Promise((resolve) => {
    const id = toast.custom(
      (t) => (
        <div
          className={`pointer-events-auto bg-white/90 backdrop-blur-xl text-slate-900 rounded-xl shadow-2xl border border-slate-200 p-4 w-80 ${t.visible ? 'animate-enter' : 'animate-leave'}`}
        >
