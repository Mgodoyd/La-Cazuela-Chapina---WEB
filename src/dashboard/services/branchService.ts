...
export interface Branch { id: string; name: string; address?: string; phone?: string; active?: boolean; createdAt?: string; }
export interface BranchResponse { status: string; data: any[]; }
...
export class BranchService {
  ...
  static async create(branch: { name: string; address?: string; phone?: string }): Promise<boolean> {
    const res = await ApiService.post('/branch/create', branch, true);
    return res?.status === 'ok';
  }
  static async update(id: string, branch: { name?: string; address?: string; phone?: string }): Promise<boolean> {
